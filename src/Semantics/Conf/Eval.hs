...
constCompare c@(Const _ c') v = case (c',v) of
  (Int v, VInt v') -> v == v'
  (Str v, VStr v') -> v == v'
...
confEval env (Con pos c) = case c of
  ECon -> \_ -> Ok env
  ConstCon c -> \v -> if constCompare c v then Ok env
    else errMsg pos ("Pattern evaluation mismatch: expected constant " ++ show c ++ " but found " ++ show v ++ " instead.")
  VarCon x -> \v -> Ok (Data.Map.insert x v env)
  TagCon t c -> \case
    VTag t' -> if t == t' then Ok env
      else errMsg pos ("Pattern evaluation mismatch: pattern expects tag " ++ t ++ ", but found " ++ t' ++ " instead.")
