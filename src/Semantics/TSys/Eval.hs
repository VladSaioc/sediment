...
tsysEval errm tenv env v ve (r@(Rule (label, pos) _ _ _ _) : rs) = case ruleEval tenv env v ve r of
...
prEval' tenv (Ok env) (Pr pos p) = case p of
  IfPr e -> expEval env e >>= \case
    VBool True -> Ok env
    VBool False -> errMsg pos "Side condition is false"
