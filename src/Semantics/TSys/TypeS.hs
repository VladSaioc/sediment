...
tsysT :: DomEnv -> TagTable -> TSEnv -> TEnv -> [Rule] -> Err ()
tsysT de tt tenv eenv rs = results (map (tsysT' de tt tenv env) rs)

tsysT' :: DomEnv -> TagTable -> TSEnv -> TEnv -> Rule -> Err ()
tsysT' de tt tenv env (Rule label c1 c2 e prs) = let
    Just (TDom d1 d2 d3) = Data.Map.lookup thisTSys tenv
  in case confT de tt tenv env c1 d1 of
    Bad msg -> Bad msg
    Ok env' -> case confT de tt tenv env' c2 d2 of
      Bad msg -> Bad msg
      Ok env'' -> case prT de tt tenv env'' prs of
          Bad msg -> Bad msg
          Ok env''' -> case expT de tt tenv env''' e of
            Bad msg -> Bad msg
            Ok _ -> Ok ()

prT :: DomEnv -> TagTable -> TSEnv -> TEnv -> [Pr] -> Err TEnv
prT de tt tenv env = Prelude.foldl (prT' de tt tenv) (Ok env)

prT' :: DomEnv -> TagTable -> TSEnv -> Err TEnv -> Pr -> Err TEnv
